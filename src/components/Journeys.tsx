...
import { IJourneyItem, sortByType, sortOrderType, IPageoptions } from '../types';
...
const Journeys = () => {
    ...
    if (!displayJourneys) {
        return (
            <>
                <h1 className="header-style"> JOURNEYS LIST
    ...
    return (
        <>
            <h1 className="header-style">JOURNEYS LIST
                <span className="span-style">
                    <Button size="sm" onClick={displayJourneysList}>{displayJourneys ? "Hide" : "Show"}</Button>
                </span>
            </h1>
                ...
                <>
                    <Table striped bordered hover size="sm" className="tableWidth">
                        <thead>
                            <tr>
                                <th style={{ width: "10%" }} className='table-header-fixed-height'><Button disabled={isLoading} className="btn-style" size="sm" variant="danger" onClick={changeSortingOrder}>{sortOrderDisplay}</Button></th>
                                <th style={{ width: "20%" }} className='table-header-fixed-height'>Departure Station <Button disabled={isLoading} className="btn-style" size="sm" name="Departure_station_name" onClick={changeSortingRow}>SORT</Button></th>
                                <th style={{ width: "20%" }} className='table-header-fixed-height'>Return Station <Button disabled={isLoading} className="btn-style" size="sm" name="Return_station_name" onClick={changeSortingRow}>SORT</Button></th>
                                <th style={{ width: "10%" }} className='table-header-fixed-height'>Dist km <Button disabled={isLoading} className="btn-style" size="sm" name="Covered_distance" onClick={changeSortingRow}>SORT</Button></th>
                                <th style={{ width: "10%" }} className='table-header-fixed-height'>Duration min <Button disabled={isLoading} className="btn-style" size="sm" name="Duration" onClick={changeSortingRow}>SORT</Button></th>
                            </tr>
                        </thead>
                        <tbody>
