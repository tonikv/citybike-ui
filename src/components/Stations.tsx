...
const Stations = () => {
  const { data: stationItems, isLoading } = useFetchData<IStationItem[]>(
    STATIONS_URL,
    []
  );
  const [selectedStation, setSelectedStation] = useState<IStationItem | null>(
    null
  );
  ...
  const memoizedStation = useMemo(() => {
    ...
    const changeStation = (event: React.ChangeEvent<HTMLSelectElement>) => {
      ...
      selectedIndex = Number.isNaN(selectedIndex)
        ? (selectedIndex = 0)
        : selectedIndex;
