...
describe("StationItem component", () => {
  test("renders station data", () => {
    ...
    const startingStationCount = screen.getByText(
      `${station.Starting_station_count}`
    );
    const endingStationCount = screen.getByText(
    ...
    const averageFrom = screen.getByText(
      /average distance traveled from this station/i
    );
    const averageTo = screen.getByText(
      /average distance traveled to this station/i
    );
    ...
    const latilongitudeText = screen.getByText(
      `latitude: ${station.y.toFixed(4)}, longitude: ${station.x.toFixed(4)}`
    );
